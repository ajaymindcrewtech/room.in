    ...
    <section class="content">
        ...
        <div class="row" style="margin-bottom: 10px">
             <!-- <div class="alert alert-success"> -->
              <form name="fillter" method="post" id="fillter" action="<?php echo base_url('report/emp_report') ?>">
      	    ...
      	    <tbody id="">
                <?php
                 ...
                 foreach ($result as $key => $value) {
                    ...
                    $cashbal=$this->Report_model->gee_res_emp_to($value->employee_id,$from,$to,1);                 
                    $phone_bal=$this->Report_model->gee_res_emp_to($value->employee_id,$from,$to,2);
                    $pyatm_bal=$this->Report_model->gee_res_emp_to($value->employee_id,$from,$to,3);
                    $bank=$this->Report_model->gee_res_emp_to($value->employee_id,$from,$to,5); 
         ...
         <tbody id="">
          <?php
           ...
           foreach ($result as $key => $value) { 
                   ...
                   $cashbal=$this->Report_model->gee_res_emp_to2($value->employee_id,$from,$to,1);
                   // $ref_cashbal=$this->Report_model->gee_res_by_emp_id_mode($value->paying_employee,$from,$to,1,2);
                  // echo $this->db->last_query();
                   $phone_bal=$this->Report_model->gee_res_emp_to2($value->employee_id,$from,$to,2);
                   // $ref_phone_bal=$this->Report_model->gee_res_by_emp_id_mode($value->paying_employee,$from,$to,2,2);
                   $pyatm_bal=$this->Report_model->gee_res_emp_to2($value->employee_id,$from,$to,3);
                   // $ref_pyatm_bal=$this->Report_model->gee_res_by_emp_id_mode($value->paying_employee,$from,$to,3,2);
                   $bank=$this->Report_model->gee_res_emp_to2($value->employee_id,$from,$to,5);  
                  // $ref_bank=$this->Report_model->gee_res_by_emp_id_mode($value->paying_employee,$from,$to,5,2);  
                    $cash_total+=$cashbal->expense;
                 
                     $phon_total+=$phone_bal->expense;
                  
                     $pyatm_total+=$pyatm_bal->expense;
                 
                      $bank_total+=$bank->expense; 

                   $total+=$cashbal->expense+$phone_bal->expense+$pyatm_bal->expense+$bank->expense;         
            ?>


                <tr>
                 <td><?php $res=$this->Employee_model->get_by_id($value->employee_id);if($res){ echo $res->name.'-('.$res->id.')';} ?></td> 
                <td ><?php if($cashbal){echo $cab=$cashbal->expense ;} else {echo '--';}?></td>
              
                <td><?php if($phone_bal){echo $pb=$phone_bal->expense ;} else {echo '--';} ?></td>
               
                <td><?php if($pyatm_bal){echo $pt=$pyatm_bal->expense;} else {echo '--';} ?></td>
               
                <td><?php if($bank){echo $ban= $bank->expense;} else {echo '--';} ?></td>
         ...
         <tbody id="">
          <?php
           ...
           foreach ($result as $key => $value) { 
                   ...
                   $cashbal=$this->Report_model->gee_res_emp_to_net($value->employee_id,$from,$to,1);
                   // $ref_cashbal=$this->Report_model->gee_res_by_emp_id_mode($value->paying_employee,$from,$to,1,2);
                  // echo $this->db->last_query();
                   $phone_bal=$this->Report_model->gee_res_emp_to_net($value->employee_id,$from,$to,2);
                   // $ref_phone_bal=$this->Report_model->gee_res_by_emp_id_mode($value->paying_employee,$from,$to,2,2);
                   $pyatm_bal=$this->Report_model->gee_res_emp_to_net($value->employee_id,$from,$to,3);
                   // $ref_pyatm_bal=$this->Report_model->gee_res_by_emp_id_mode($value->paying_employee,$from,$to,3,2);
                   $bank=$this->Report_model->gee_res_emp_to_net($value->employee_id,$from,$to,5);  
                  // $ref_bank=$this->Report_model->gee_res_by_emp_id_mode($value->paying_employee,$from,$to,5,2);  
                   ...
                   // $total+=$cashbal->income - $cashbal->expense+$phone_bal->income - $phone_bal->expense +$pyatm_bal->income - $pyatm_bal->expense + $bank->income - $bank->expense;      
            ?>
