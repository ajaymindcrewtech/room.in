...
class Antispam {
	...
	*/
	...
	var $char_length 	= 	3;		// CODE_CHARS_COUNT
	...
	function get_antispam_image( $override = array() ) {
			...
			foreach ( $override as  $key => $value) {
				...
				if( isset( $this->$key ))
					$this->$key = $value;
                                 ...
                                 if($this->img_name!=''){
                                    $REALPATH=FCPATH.$this->img_path.$this->img_name;
		...
		return array('word' => $code, 'time' => $now, 'image' => $img_markup,'name'=>$img_name);
